...
- {@class https://uor.foundation/proof/InductiveProof} -- proved by induction
  over quantum levels. The base case holds at some minimal level, and the
...
- {@class https://uor.foundation/proof/EmpiricalVerification} -- verified by
  quantum-level measurement. Used for identities that involve superposition
...
- {@prop https://uor.foundation/proof/atQuantumLevel} -- the quantum level at
  which the proof is valid.
- {@prop https://uor.foundation/proof/universalScope} -- whether the proof
  holds at all quantum levels (true for Universal scope).
...
- {@class https://uor.foundation/cert/SaturationCertificate} -- certifies that
  the evaluation context is fully saturated (all bindings resolved).
...
See {@concept quantum-levels} for how proofs are scoped to specific ring scales,
